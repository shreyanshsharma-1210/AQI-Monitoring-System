...
const CustomTooltip = ({ active, payload, label, unit }) => {
  ...
  return (
    <div className="bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-xs">
      <p className="text-gray-400 mb-1">{payload[0]?.payload?.recorded_at?.slice(0, 16) || label}</p>
      <p className="font-semibold text-white">{payload[0]?.value?.toFixed(2)} {unit}</p>
    </div>
  );
};
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="max-w-5xl mx-auto px-4 py-5 space-y-5">
        ...
        <div className="bg-gray-900 rounded-xl border border-gray-700 p-4">
          <p className="text-xs text-gray-400 mb-3">Switch pollutant</p>
        ...
        <div className="bg-gray-900 rounded-xl border border-gray-700 p-5">
          ...
          {loading ? (
            ...
            <div className="h-64 flex items-center justify-center text-gray-500 text-sm">
              No data yet — pipeline runs every 15 minutes
            </div>
          ) : (
